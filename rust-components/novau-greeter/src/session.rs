...
pub fn enumerate() -> Result<Vec<Session>> {
    ...
    for dir in SESSION_DIRS {
        ...
        let kind = if dir.ends_with("xsessions") {
            SessionKind::X11
        } else {
            SessionKind::Wayland
        };
        for entry in std::fs::read_dir(p)? {
            ...
            if let Some(s) = parse_desktop_file(&path, kind) {
                out.push(s);
            }
        }
    }
...
pub fn spawn(uid: u32, user: &str, session: &Session) -> Result<()> {
    ...
    let _pam = PamSession::open(user, "").map_err(|e| anyhow!("pam_open_session: {e}"))?;

    // Build env vector
    let mut env: Vec<(&str, String)> = Vec::new();
    env.push((
        "XDG_SESSION_TYPE",
        if session.kind == SessionKind::Wayland {
            "wayland".into()
        } else {
            "x11".into()
        },
